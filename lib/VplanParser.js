...
const Templates = {
  [VplanType.STUDENTS]: {
    ...
    info: ['vp.fuss.fusszeile.fussinfo._', info => (info.match(/(.*) TICKER=/) ? info.match(/(.*) TICKER=/)[1] : undefined || '')],
    ticker: ['vp.fuss.fusszeile.fussinfo._', info => (info.match(/TICKER=(.*)/) ? info.match(/TICKER=(.*)/)[1] : undefined || '')]
  },
...
const VplanParser = {
  getType(rawVplan) {
    const rawInstructionStylesheet = rawVplan._instruction['xml-stylesheet']
      .split('href="')[1]
      .split('"')[0]
  ...
  async parse(rawVplan) {
    ...
    vplan = Object.assign({ type: vplanType }, vplan)
  ...
  getQueueDay(vplan) {
    ...
    log.info('AUTO_SCHEDULING', `${vplan.type}: [${queueDateString}]`)
