<template>
  <div class="overflow-hidden position-relative">
    ...
    >
      ...
      >
        <octicon icon="alert" :scale="5"></octicon>
        <octicon icon="screen-normal" :scale="3"></octicon>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    populateTable(pageNr = 0) {
      ...
      console.debug({ PagingVplanTable: "PAGE_CHUNKS", head, end: head + length, length, displayTime })

      this.$store.commit("SET_VPLAN_PAGE", { queue: this.queue, pageNr })

      this.populateTableCycleID = setTimeout(() => {
        pageNr++
        pageNr < this.vplanPageChunks.length
